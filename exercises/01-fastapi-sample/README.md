...
1. Python 3.13 以降を使用してください。
1. ライブラリの追加は行わないでください。
1. 各種生成AIツール（ChatGPT, Claude Code, GitHub Copilotなど）を開発に活用いただくことは問題ありません。
   - 使用の有無や、注意した点・工夫した点などについては、面接時に質問させていただく場合があります。
   - `AGENTS.md` や `CLAUDE.md` など、生成AIに対する指示や定義を作成した場合は、併せてコミットし、提出物に含めてください。
1. 提出時には回答内容の全てが`submit`という名前のブランチに反映されている状態にしてください。提出後はmainブランチとの差分を確認します。
...
デフォルトでは Python 3.13 のイメージを作成します。別のバージョンを利用したい場合、環境変数 `PYTHON_VERSION` に任意のバージョンを指定してください。以下は Python 3.14 の例です。

```bash
$ PYTHON_VERSION=3.14 make docker-build
```

イメージのビルド後、以下のコマンドでテストを実行します。

```bash
$ make docker-run-unit         # ユニットテスト実行
$ make docker-run-integration  # 統合テスト実行
$ make docker-run              # 全テストを実行
```
...
API にユーザ認証機能を実装してください。認証実装後は `X-API-TOKEN` をリクエストヘッダに入れる事でユーザ認証を行う事とします。ただし、ヘルスチェック用エンドポイント (`GET /health-check`) 、ユーザ作成エンドポイント (`POST /users`) は無認証で受け付ける事とします。`X-API-TOKEN` はユーザ作成時に発行し、レスポンスに含めます。
...
ユーザの削除を行うエンドポイントを追加してください。エンドポイントの仕様を以下に示します。
また、必要なテストケースの追加を行ってください。

### 仕様

- メソッド・パスは`DELETE /users/:user_id`
- ユーザの削除は、`is_active` を `False` にする事で行う
- 削除対象のユーザが所有する `item` は、有効なユーザかつ `id` が最も小さなユーザに所有権を移す
...
`submit.sh` を実行して、全テストの実施と提出物作成を行います。

- bashとDockerが実行可能な環境で実行してください。
- 回答内容が全てsubmitブランチに反映されている状態にしてください。提出後はmainブランチとの差分を確認します。
- 未コミットのファイルがある場合はスクリプト実行時にエラーになります。
- テスト実行に失敗した場合はスクリプト実行時にエラーになります。
  - エラーが発生した場合はテストが通るように回答内容を修正した上で再度`submit.sh` を実行してください。
...
実行後、`submit.zip`というファイルが生成されるので、メールで提出してください。
